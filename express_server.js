...
app.get("/urls", (req, res) => {
  const templateVars = {
    users,
    username: getUsername(req.session["user_id"], users),
...
app.get("/urls/:shortURL", (req, res) => {
  ...
  if (
    req.session["user_id"] &&
    req.session["user_id"] === urlDatabase[shortURL]["userID"]
  ) {
    const templateVars = {
      users,
      username: getUsername(req.session["user_id"], users),
    ...
    return res
      .status(401)
      .send(
        "Cannot view - you either do not own this short URL or are not logged in."
      );
  }
});
...
app.post("/urls/:shortURL/delete", (req, res) => {
  ...
  if (
    ...
    return res
      .status(401)
      .send("Cannot delete - you do not own this short URL.");
  }
});
...
app.post("/login", (req, res) => {
  ...
  if (
    ...
    bcrypt.compareSync(password, users[getUserByEmail(email, users)].password)
  ) {
...
app.post("/register", (req, res) => {
  ...
  if (checkForUsername(email, users) || email === "" || password === "") {
    return res
      .status(400)
      .send(
        "Error. Ensure you have entered a username and password and are not already registered."
      );
  } else {
